...
export async function GET(
  ...
  if (!idValidation.success) {
    ...
    const raw = await OwnerRepository.getOwner(idValidation.data);
...
export async function DELETE(
  ...
  if (!idValidation.success) {
    ...
    const raw = await OwnerRepository.deleteOwner(idValidation.data);
...
export async function POST(
  ...
  if (!idValidation.success || !requestValidation.success) {
    ...
    const raw = await OwnerRepository.updateOwner(requestValidation.data, idValidation.data);
    const data = OwnerTransformer.MOwnerTransformer(raw);
